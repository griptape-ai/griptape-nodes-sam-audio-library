...
def _install_perception_models_no_deps() -> None:
    ...
    It cannot be declared in `pip_dependencies_exec`: its closure pulls `torchcodec`, which the
    manifest's CUDA wheel index carries with no Windows wheel, and `pip_install_flags` applies to
    every requirement in both sets so `--no-deps` cannot be scoped to this one package.
    """
    if sys.platform != "win32":
        return
...
def _setup_torchcodec_mock() -> None:
    ...
    resolve even though the manifest's CUDA wheel index publishes no Windows build of it.
    """
    if sys.platform != "win32":
        return
...
class SamAudioLibraryAdvanced(AdvancedNodeLibrary):
    ...
    def _install_sam_audio(self, sam_audio_path: Path) -> None:
        ...
        own metadata requires `perception-models` and `torchcodec`, neither of which resolves for
        Windows against the manifest's CUDA wheel index.
        """
